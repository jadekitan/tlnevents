...
import { useNavigate } from "react-router-dom"; // Changed from useRoutes to useNavigate
...
const Page = () => {
  ...
  const [formValues, setFormValues] = useState({
    email: "",
    eventName: "", // Initialize as empty string
  });
  ...
  // Update formValues when selectedEvent changes
  useEffect(() => {
    setFormValues((prev) => ({
      ...prev,
  ...
  const getAffiliate = async (userData) => {
    ...
    try {
      ...
      const response = await apiClient.get(
        `/affiliates?email=${encodeURIComponent(
          apiRequestData.email
        )}&eventName=${encodeURIComponent(selectedEvent)}`
      );
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      try {
        const response = await getAffiliate(formValues); // Fixed function name

        toast({
          position: "top",
          title: "Affiliate Logged In.",
          description:
        ...
        // Fixed navigation - use navigate instead of useRoutes
        navigate(
          `/affiliate/dashboard?email=${encodeURIComponent(
            formValues.email
          )}&eventName=${encodeURIComponent(selectedEvent)}`
        );
      } catch (error) {
          ...
          >
            <VStack justify="center" align="center" gap={["4px", "10px"]}>
              ...
              <Text color="neutral.500" fontSize={["12px", "16px"]}>
                Enter your details to access your affiliate dashboard
              </Text>
            </VStack>
